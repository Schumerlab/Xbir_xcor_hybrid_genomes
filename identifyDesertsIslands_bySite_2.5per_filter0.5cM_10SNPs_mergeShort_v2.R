...
} else if (file.exists(paste("average_average_ancestry_by_site_", popName, ".txt_ancestry_xbir_genome_0.05cM_windows.bed_WG", sep=""))) {
  print("0.05cM windowed data for focal population exists and full pipeline will run")
} else {
...
if (file.exists("average_ancestry_allPops_codeConservedSynNonSyn_0.05cM_windows.txt")) {
  ...
  cMdata <- read.table(paste("average_average_ancestry_by_site_", popName, ".txt_ancestry_xbir_genome_0.05cM_windows.bed_WG", sep=""), header=F)
  header <- c("chr", "start", "end", "HI", "numInds", "nSNPs")
  meanHI <- mean(cMdata$V4, na.rm=T)
  #make everything in terms of minor parent ancestry
  if (meanHI > 0.5) {
    #cMdata$majPar <- cMdata$V4
    cMdata$minPar <- 1-cMdata$V4
  } else {
    #cMdata$majPar <- 1-cMdata$V4
    cMdata$minPar <- cMdata$V4
  }
  #genomeData$popMin <- cMdata$minPar
  header <- c(header, popName)
  colnames(cMdata) <- header
}
...
if (file.exists("minorParentQuantiles_allPops_0.05cMwins.txt")) {
  ...
  if (file.exists("average_ancestry_allPops_codeConservedSynNonSyn_0.05cM_windows.txt")) {
    ...
    popAnc <- cMdata[,ncol(cMdata)]
    popQuants <- quantile(popAnc, c(0.01, 0.025, 0.05, 0.1, 0.9, 0.95, 0.975, 0.99), na.rm=T)
    tmpRow <- data.frame(pop=popName, meanMinPar=mean(popAnc, na.rm = T), lower1=popQuants[[1]], lower2.5=popQuants[[2]], lower5=popQuants[[3]], lower10=popQuants[[4]], upper10=popQuants[[5]], upper5=popQuants[[6]], upper2.5=popQuants[[7]], upper1=popQuants[[8]])
    cMquantiles <- tmpRow
    write.table(cMquantiles, paste("minorParentQuantiles_", popName, "_0.05cMwins.txt", sep=""), row.names = F, quote=F, sep="\t")
  }
}
